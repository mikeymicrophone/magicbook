class MugglesController < ApplicationController
  ...
  def submit
    ...
    BookMailer.gifted(@purchase.id, @muggle_1.id, params[:message]).deliver_later if @muggle_1.valid?
    BookMailer.gifted(@purchase.id, @muggle_2.id, params[:message]).deliver_later if @muggle_2.valid?
    BookMailer.gifted(@purchase.id, @muggle_3.id, params[:message]).deliver_later if @muggle_3.valid?
    BookMailer.gifted(@purchase.id, @muggle_4.id, params[:message]).deliver_later if @muggle_4.valid?
  end
